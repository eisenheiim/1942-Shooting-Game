...
public class Bonus {
    ...
    //player's bullets
    static double[] enemyBulletDir = new double[100]; // +1 up, -1 down
    ...
    static double[] enemyspeedxd = new double[8];
    static double enemyspeedyd ;
    ...
    static void startGame(){
        ...
        for(int i=0; i<enemy_number;i++){
            ...
            if (i<4){
                ...
                enemyX[i]=canvas_width/9+(i-4)*canvas_width/9*2-50;
...
//moving bullets,hearts
    static void updateEnemyBullets() {
        for (int i = 0; i < enemyBulletCount; i++) {
            if (enemyBulletActive[i]) {
                enemyBulletY[i] += bullet_speed * enemyBulletDir[i];
                if (enemyBulletY[i] < 0) {
    ...
    static void updateenemies() {
        ...
        // frame counter (used for smooth motion)
        enemy_move++;
        double disappearChance = 0.002;
        int minHiddenFrames = 40;          // minimum time hidden
        int maxHiddenFrames = 140;         // maximum time hidden

        for (int i = 0; i < enemy_number; i++) {
            if (!enemyActive[i]) continue;

            //  If enemy is hidden (i hide by moving outside off the off-screen)
            //    i use enemyShootTimer[i] as a countdown until it returns

            if (enemyY[i] < 0) {
                enemyShootTimer[i]--; // count down

                if (enemyShootTimer[i] <= 0) {
                    // respawn near top in 2 rows
                    enemyX[i] = (canvas_width / 10.0);
                    enemyY[i] = canvas_height - enemy_height / 2.0;

                    enemyShootTimer[i] = 0; // restart orbit time
                }
                continue;
            }
            ...
            double time = enemyShootTimer[i];
            double angle = 0.03 * time + i; //

            // small drifting center
            double centerX = enemyX[i] + 0.15 * Math.sin(0.02 * enemy_move + i);
            double centerY = enemyY[i] + 0.15 * Math.cos(0.017 * enemy_move + i);

            enemyX[i] = centerX + 20 * Math.cos(angle);
            enemyY[i] = centerY + 20 * Math.sin(angle);

            // keep inside screen
            enemyX[i] = Math.max(enemy_width / 2.0, Math.min(canvas_width - enemy_width / 2.0, enemyX[i]));
            enemyY[i] = Math.max(enemy_height / 2.0, Math.min(canvas_height - enemy_height / 2.0, enemyY[i]));

            // Random disappearance (but NOT death)
            if (Math.random() < disappearChance) {
                enemyY[i] = -1000; // hidden off-screen
                enemyShootTimer[i] = minHiddenFrames
                        ;
            }
            // shooting (simple, independent from timers)
            if (Math.random() < 0.02) {
...
//this method checks rectangle collision
    ...
    static boolean allEnemiesDefeated() {
        for (int i = 0; i < enemy_number; i++) {
            if (enemyActive[i]) {
                return false;
            }
    ...
    static void drawenemy(){
        for(int i=0;i<enemy_number;i++){
            if(enemyActive[i]){
                StdDraw.picture(enemyX[i],enemyY[i],"assets/196-1967288_space-ship-enemy-space-ship-enemies-png.png",enemy_width,enemy_height);
        ...
        for (int i = 0; i < explosionNumber; i++) {
            if (explosionActive[i]) {
                if (eplosion_timer[i] < 10) {
                    StdDraw.picture(explosionX[i], explosionY[i], "assets/images.jpeg", 60, 60);
                } else {
                    StdDraw.picture(explosionX[i], explosionY[i], "assets/images.jpeg", 80, 80);
                }
            }
        }
    }
...
static void addEnemyBullet(double x, double y) {
    ...
    if (enemyBulletCount >= enemyBulletX.length) return;

    enemyBulletX[enemyBulletCount] = x;
    enemyBulletY[enemyBulletCount] = y;
    enemyBulletActive[enemyBulletCount] = true;

    //  If enemy is below player, shoot upward
    if (y < playery) {
        enemyBulletDir[enemyBulletCount] = 1;   // go up
    } else {
        enemyBulletDir[enemyBulletCount] = -1;  // go down
    }
